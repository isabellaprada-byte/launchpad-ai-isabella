...
async function uploadArtifact(
  ...
  const prepJson = await prepRes.json() as Record<string, unknown>;
  console.log(`artifacts.prepare response keys: ${Object.keys(prepJson).join(', ')}`);

  const artifactId: string = (prepJson.artifact as Record<string,string>)?.id ?? prepJson.artifact_id as string ?? prepJson.id as string;
  const uploadUrl: string = prepJson.upload_url as string ?? prepJson.url as string;
  const formData: Array<{ key: string; value: string }> = prepJson.form_data as Array<{ key: string; value: string }> ?? [];
