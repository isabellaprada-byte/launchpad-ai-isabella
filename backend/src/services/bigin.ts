// Bigin (Zoho CRM) API — token refresh + contact search
...
export async function searchContacts(query: string): Promise<BiginContact[]> {
  ...
  // Bigin uses `word` for full-text search across all fields
  const params = new URLSearchParams({
    word:     query,
  ...
  if (!res.ok) {
    ...
    throw new Error(`Bigin search failed (${res.status}): ${err}`);
  }

  const data = await res.json() as { data?: Record<string, unknown>[] };
  if (!data.data) return [];

  // Enrich each contact with plan type from their company's deal
  const contacts = await Promise.all(data.data.map(async c => {
